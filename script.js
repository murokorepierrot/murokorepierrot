    ...
    function initClickToFrontCards(selector) {
      ...
      function closeCard() {
        ...
        setTimeout(function() {
          ...
          activeCard.classList.remove('popup-enter', 'popup-exit');
...
(function() {
  ...
  if (isStandalone) {
    // App is already installed — hide the banner permanently
  ...
  // Show banner with a delay so it doesn't interrupt the user
  function showBanner() {
    if (!installBanner) return;
    // Show after 3 seconds, but only if not dismissed before
    setTimeout(function() {
      // Check if user dismissed it earlier
      const dismissed = localStorage.getItem('marierose_install_dismissed');
      if (!dismissed) {
  ...
  // iOS / Safari: detect if it's iOS and show a different message
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream;
  if (isIOS && !isStandalone) {
    // Show iOS-specific instruction
    const textSpan = installBanner?.querySelector('.install-banner-text span');
    if (textSpan) {
      textSpan.textContent = 'Tap Share → Add to Home Screen to install this app.';
    }
    // iOS doesn't have beforeinstallprompt, so show banner
    showBanner();
  }
      ...
      if (deferredPrompt) {
        ...
        deferredPrompt.userChoice.then(function(choiceResult) {
          if (choiceResult.outcome === 'accepted') {
            ...
            localStorage.setItem('marierose_install_dismissed', 'true');
          }
          deferredPrompt = null;
        });
      } else if (isIOS) {
        // iOS — just show instructions (already shown)
        installBanner.classList.remove('show');
        localStorage.setItem('marierose_install_dismissed', 'true');
      }
    });
  ...
  // Hide if app is already installed via display-mode change
  window.addEventListener('appinstalled', function() {
    installBanner.classList.remove('show');
    localStorage.setItem('marierose_install_dismissed', 'true');
  });

  // If the banner somehow stays visible after 15 seconds, hide it
  setTimeout(function() {
    if (installBanner && installBanner.classList.contains('show')) {
      // Only hide if it's been shown for a while and user hasn't interacted
    }
  }, 15000);
